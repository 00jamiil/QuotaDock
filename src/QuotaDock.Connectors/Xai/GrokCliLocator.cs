...
/// <summary>
/// Finds a launchable Grok Build CLI on this machine. Mirrors
...
/// </summary>
public sealed class GrokCliLocator
{
    private static readonly TimeSpan ProbeTimeout = TimeSpan.FromSeconds(10);
    private static readonly string[] AllowedFileNames = ["grok.exe", "grok.cmd"];
    private readonly Func<string, bool> fileExists;
    private readonly Func<string, CancellationToken, Task<bool>> probe;
    private readonly Func<string, string?> getEnvironmentVariable;

    public GrokCliLocator()
        ...
        CancellationToken cancellationToken = default)
    {
        foreach (var candidate in Candidates(configuredPath).Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (!IsGrokExecutable(candidate) || !fileExists(candidate))
            {
                continue;
            }
    ...
    private IEnumerable<string> Candidates(string? configuredPath)
    {
        ...
        {
            yield return Path.Combine(userProfile, ".local", "bin", "grok.exe");
        }

        var appData = getEnvironmentVariable("APPDATA");
        if (!string.IsNullOrWhiteSpace(appData))
        {
            yield return Path.Combine(appData, "npm", "grok.cmd");
        }
    }

    private static bool IsGrokExecutable(string path) =>
